...
START_EARLY_TOL = int(CONFIG.get("start_early_tol", 240))
START_LATE_TOL = int(CONFIG.get("start_late_tol", 360))
END_EARLY_TOL = int(CONFIG.get("end_early_tol", 360))
END_LATE_TOL = int(CONFIG.get("end_late_tol", 360))
...
def _normalize_columns(df):
    ...
    emp_col = None
    for c in ["empcode", "personcode", "employeeid", "employee_id"]:
        if c in cols:
            emp_col = cols[c]
            break

    dt_col = None
    for c in ["transactiondatetime", "transaction_date_time", "datetime", "punchdatetime"]:
        if c in cols:
            dt_col = cols[c]
            break
...
def _choose_best_reader_candidate(win_df):
    if win_df.empty:
        return None, None, []
    ...
    for sh in MODELED_SHIFT_CLASSES:
        ...
        start_lo = float(meta["start_min"]) - START_EARLY_TOL
        start_hi = float(meta["start_min"]) + START_LATE_TOL

        end_lo = float(meta["end_min"]) - END_EARLY_TOL
        end_hi = float(meta["end_min"]) + END_LATE_TOL

        in_df = win_df[
            (win_df["reader_number"] == 1) &
            (win_df["minute_from_day0"] >= start_lo) &
            (win_df["minute_from_day0"] <= start_hi)
        ].copy()

        out_df = win_df[
            (win_df["reader_number"] == 2) &
            (win_df["minute_from_day0"] >= end_lo) &
            (win_df["minute_from_day0"] <= end_hi)
        ].copy()
        ...
        valid_out_df = out_df[out_df["ts"] > in_time].copy()

        if valid_out_df.empty:
            continue

        out_row = valid_out_df.sort_values("ts").iloc[-1]
        ...
        outside_punch_count = int(len(win_df) - pair_punch_count)
        ...
        candidates.append({
            ...
            "pair_punch_count": pair_punch_count,
            "outside_punch_count": outside_punch_count,
            "reader_in_time": in_time,
            "reader_out_time": out_time,
            "reader_pair_method": "reader1_earliest_reader2_latest",
        })
...
def _extract_reader_features(win_df, day0, best, second):
    base = {
...
def _final_decision(row):
    ...
    if conf >= 0.98 and model_pred == pair_pred and _valid_model_shift(model_pred):
        return (
            ...
            "Model and reader-pair candidate strongly agree"
        )
    ...
    if (
        ...
        return (
            ...
            "Overnight reader-pair pattern agrees with model"
        )
    ...
    if conf >= 0.85 and model_pred == pair_pred and _valid_model_shift(model_pred):
        return (
            ...
            "Model and reader-pair candidate agree, but review recommended"
        )
    ...
    if (
        ...
        return (
            ...
            "SHIFT_REVIEW",
            "pair_strong_review",
            "Strong reader-pair candidate found; review recommended"
        )
    ...
    if overtime_like == 1 and conf >= 0.80 and _valid_model_shift(model_pred):
        return (
            ...
            "SHIFT_REVIEW",
            "overtime_review",
            "Overtime-like reader-pair span found; review recommended"
        )
...
def run_inference(input_file_path, output_dir=None, return_clean=True):
    ...
    for row in base[["PersonCode_norm", "attendance_day"]].itertuples(index=False):
        ...
        best, second, _ = _choose_best_reader_candidate(win_df)
        feat = _extract_reader_features(win_df, day0, best, second)
